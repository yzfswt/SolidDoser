...
# !/usr/bin/env python
"""
mod: "serial_labware" -- Generic base class for communication whit lab equipment via serial connection
===============================================================
module:: serial_labware
platform:: Windows
synopsis:: Generic base class to control lab equipment via serial

This provides a generic python class for safe serial communication with various lab equipment over
serial interfaces (RS232, RS485, USB) by sending command strings. This parent class handles establishing
...
def command(func):
    ...
    handler therad and actually executes the method. This way methods in the child classes need to be written just one
...
class SerialDevice:
    ...
    def __init__(self, port=None, device_name=None, soft_fail_for_testing=False):
        """
        :param port: The port name/number of the serial device
        :param device_name: A descriptive name for the device, used mainly in debug prints.
        :param soft_fail_for_testing: (optional) determines if an invalid serial port raises an error or merely
        log a message. Default: Off
        """
        ...
        # DEBUG testing switch, to allow soft_fails instead of exceptions
        ...
        if platform.system() == "Windows":
            try:
                if port[0:3] == "COM" and int(port[3:]) > 0:
                    ...
                    if not self.__soft_fail_for_testing:
                        # in normal use raise an exception to make surrounding python scripts stop
            ...
            raise (OSError(
                "You are running a currently unsupported operating system: \"{0}\"".format(
                    platform.system())
            ))
        ...
        # syntax of a returned answer, to bo overridden by child classes
    ...
    def open_connection(self):
        ...
        try:
            ...
            print(self.__connection.is_open)
        ...
        if self.__connection is not None:
            with self._query_lock:
                ...
                if get_return:
                    ...
                    output = self.__receive_message(return_pattern=return_pattern, multiline=multiline)
                else:
        ...
        try:
            ...
            if self.__connection is not None:
                ...
                if return_pattern is not None:
                    if type(return_pattern).__name__ != "Pattern":
                        raise ValueError(
                    ...
                    if re.fullmatch(pattern=return_pattern, string=answer):
                        ...
                        return re.match(pattern=return_pattern, string=answer).groups()[0]
                    else:
